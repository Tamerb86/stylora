...
export default function QuickBookingDialog({ open, onOpenChange }: QuickBookingDialogProps) {
  const { t } = useTranslation();
  const [currentStep, setCurrentStep] = useState<"customer" | "service" | "datetime" | "confirm">("customer");
  ...
  const handleClose = () => {
    ...
    setSearchQuery("");
    setBookingForm({ serviceId: "", employeeId: "", date: new Date(), time: "" });
    onOpenChange(false);
  };
  ...
  const selectedService = services?.find((s: any) => s.id === parseInt(bookingForm.serviceId));
  const selectedEmployee = employees?.find((e: any) => e.id === parseInt(bookingForm.employeeId));
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          {[
            { key: "customer", icon: User, label: t("quickBooking.customer", "Customer") },
            { key: "service", icon: CalendarIcon, label: t("quickBooking.service", "Service") },
            { key: "datetime", icon: Clock, label: t("quickBooking.dateTime", "Date & Time") },
            { key: "confirm", icon: CheckCircle, label: t("quickBooking.confirm", "Confirm") },
          ].map((step, index) => {
            const Icon = step.icon;
            const isActive = step.key === currentStep;
            const isCompleted = ["customer", "service", "datetime", "confirm"].indexOf(step.key) < 
                               ["customer", "service", "datetime", "confirm"].indexOf(currentStep);
          ...
          <div className="space-y-4">
            <Tabs defaultValue="existing" className="w-full">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="existing">{t("quickBooking.existingCustomer", "Existing Customer")}</TabsTrigger>
                <TabsTrigger value="new">{t("quickBooking.newCustomer", "New Customer")}</TabsTrigger>
              </TabsList>

              <TabsContent value="existing" className="space-y-4">
                <div className="space-y-2">
                  <Label>{t("quickBooking.searchCustomer", "Search Customer")}</Label>
                  <Input
                    placeholder={t("quickBooking.searchPlaceholder", "Name or phone...")}
                    value={searchQuery}
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button onClick={handleCustomerNext}>
                {t("common.next")}
              </Button>
            </div>
          </div>
        ...
        {currentStep === "service" && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={bookingForm.employeeId} onValueChange={(value) => setBookingForm({ ...bookingForm, employeeId: value })}>
                <SelectTrigger>
                  <SelectValue placeholder={t("appointments.selectEmployee")} />
                </SelectTrigger>
                <SelectContent>
                  {employees?.filter((e: any) => e.isActive).map((employee: any) => (
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Calendar
                ...
                onSelect={(date) => date && setBookingForm({ ...bookingForm, date })}
        ...
        {currentStep === "confirm" && (
          <div className="space-y-4">
            <div className="bg-gradient-to-br from-blue-50 to-purple-50 p-6 rounded-lg space-y-4">
              <h3 className="font-semibold text-lg">{t("quickBooking.summary", "Booking Summary")}</h3>
              
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t("quickBooking.customer")}:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t("appointments.employee")}:</span>
                  <span className="font-semibold">{selectedEmployee?.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t("appointments.date")}:</span>
                  <span className="font-semibold">{bookingForm.date.toLocaleDateString('no-NO')}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between pt-2 border-t">
                  ...
                  <span className="font-bold text-lg">{selectedService?.price} kr</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between gap-2 pt-4">
              ...
              >
                {createAppointmentMutation.isPending ? t("common.creating") : t("quickBooking.confirmBooking", "Confirm Booking")}
              </Button>
            </div>
          </div>
        )}
