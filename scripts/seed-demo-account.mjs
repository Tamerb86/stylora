...
const { tenants, users, services, customers, appointments, serviceCategories } = schema;
...
try {
  ...
  const existingTenant = await db
    .select()
    .from(tenants)
    .where(schema.eq(tenants.id, DEMO_TENANT_ID))
    .limit(1);
  ...
  const existingUser = await db
    .select()
    .from(users)
    .where(schema.eq(users.email, DEMO_USER_EMAIL))
    .limit(1);
  ...
  for (const emp of employees) {
    const existing = await db
      .select()
      .from(users)
      .where(schema.eq(users.email, emp.email))
      .limit(1);
  ...
  for (const cat of categories) {
    const existing = await db
      ...
      .where(schema.and(
        schema.eq(serviceCategories.tenantId, DEMO_TENANT_ID),
        schema.eq(serviceCategories.name, cat.name)
      ))
      .limit(1);
  ...
  for (const svc of servicesData) {
    const existing = await db
      ...
      .from(services)
      .where(schema.and(
        schema.eq(services.tenantId, DEMO_TENANT_ID),
        schema.eq(services.name, svc.name)
      ))
      .limit(1);
  ...
  for (const cust of customersData) {
    const existing = await db
      ...
      .from(customers)
      .where(schema.and(
        schema.eq(customers.tenantId, DEMO_TENANT_ID),
        schema.eq(customers.email, cust.email)
      ))
      .limit(1);
  ...
  for (const apt of appointmentsData) {
    ...
    const existing = await db
      ...
      .from(appointments)
      .where(schema.and(
        schema.eq(appointments.tenantId, DEMO_TENANT_ID),
        schema.eq(appointments.customerId, apt.customerId),
        schema.sql`DATE(${appointments.appointmentDate}) = ${dateStr}`,
        schema.eq(appointments.startTime, apt.startTime)
      ))
      .limit(1);
