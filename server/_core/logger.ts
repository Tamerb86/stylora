...
export const logAuth = {
  ...
  loginFailed: (email: string, reason: string, ip?: string) => {
    logger.warn('Login failed', { email, reason, ip, event: 'auth.login.failed' });
  },
  logout: (email: string, ip?: string) => {
    logger.info('User logged out', { email, ip, event: 'auth.logout' });
  },
  tokenExpired: (email?: string) => {
    logger.info('Session token expired', { email, event: 'auth.token.expired' });
  },
};
...
export const logPayment = {
  created: (paymentId: number, amount: string, method: string, tenantId: string) => {
    logger.info('Payment created', { paymentId, amount, method, tenantId, event: 'payment.created' });
  },
  completed: (paymentId: number, tenantId: string) => {
    logger.info('Payment completed', { paymentId, tenantId, event: 'payment.completed' });
  },
  failed: (paymentId: number, reason: string, tenantId: string) => {
    logger.error('Payment failed', { paymentId, reason, tenantId, event: 'payment.failed' });
  },
  refunded: (paymentId: number, amount: string, tenantId: string) => {
    logger.info('Payment refunded', { paymentId, amount, tenantId, event: 'payment.refunded' });
  },
};

// Log security events
export const logSecurity = {
  tenantIsolationViolation: (tenantId: string, attemptedResource: string, resourceTenantId: string) => {
