  ...
  auth: router({
    ...
    login: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!user || !user.passwordHash) {
          throw new TRPCError({ code: "UNAUTHORIZED", message: "Ugyldig e-post eller passord" });
        }
        ...
        // Check if tenant exists and email is verified
        if (user.tenantId) {
          const [tenant] = await dbInstance
            .select()
            .from(tenants)
    ...
    register: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (existingUser) {
          throw new TRPCError({ code: "CONFLICT", message: "En bruker med denne e-posten eksisterer allerede" });
        }
        ...
        await dbInstance.insert(tenants).values({
          id: tenantId,
          name: input.salonName,
          subdomain: input.salonName.toLowerCase().replace(/[^a-z0-9]/g, '-').substring(0, 30),
          emailVerified: false,
        ...
        return { 
          success: true, 
          message: "Registrering vellykket! Vennligst bekreft e-posten din.",
