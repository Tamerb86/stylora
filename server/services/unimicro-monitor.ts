  ...
  try {
    ...
    const totalDuration = completedSyncs.reduce((sum, log) => {
      if (log.syncCompletedAt && log.syncStartedAt) {
        return sum + (log.syncCompletedAt.getTime() - log.syncStartedAt.getTime());
      }
      return sum;
    }, 0);
    ...
    logDb.error('Failed to get Unimicro sync metrics', { error, tenantId, hoursAgo });
    throw error;
  }
}
 ...
 */
export async function checkUnimicroSyncHealth(tenantId: string): Promise<SyncAlert[]> {
  ...
  try {
    ...
    const consecutiveFailures = recentSyncs.filter(log => log.status === 'failed').length;
    ...
    logDb.error('Failed to check Unimicro sync health', error instanceof Error ? error : new Error(String(error)));
    return [{
      severity: 'error',
...
export async function getRecentSyncFailures(
  ...
  try {
    ...
    logDb.error('Failed to get recent sync failures', { error, tenantId, limit });
    throw error;
  }
}
...
export async function getSyncStatsByType(
  ...
  try {
    ...
    for (const [type, logs] of Object.entries(statsByType)) {
      ...
      const lastSync = logs.sort((a, b) => 
        b.syncStartedAt.getTime() - a.syncStartedAt.getTime()
      )[0];
    ...
    logDb.error('Failed to get sync stats by type', { error, tenantId, hoursAgo });
    throw error;
  }
}
...
export async function scheduleUnimicroMonitoring(): Promise<void> {
  try {
    ...
    logDb.error('Failed to schedule Unimicro monitoring', error instanceof Error ? error : new Error(String(error)));
  }
}
